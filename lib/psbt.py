...
class PSBT_Role:
    ...
    def b64_psbt(self):
        '''Returns a string Base64 encoding of the PSBT'''
        return b64encode(self.serialized()).decode("utf-8") 
...
class Creator(PSBT_Role):
    ...
    def __init__(self, inputs, outputs, tx_version=2, input_sequence=0xffffffff, locktime=0, testnet=True):
        print(inputs)
        print(outputs)
        ...
        for i in inputs:
            self.tx_inputs.append(TxIn(prev_tx=i[0], prev_index=i[1], script_pubkey=b'', sequence=input_sequence, witness_program=b'\x00'))
        # Initialize list of TxOut objects (Outputs)
        for i in outputs:
            # print(TxOut(amount=i[0], script_pubkey=i[1]))
            self.tx_outputs.append(TxOut(amount=i[0], script_pubkey=i[1]))
        self.tx_obj = Tx(version=tx_version, tx_ins=self.tx_inputs, tx_outs=self.tx_outputs, locktime=locktime, testnet=testnet)
...
class Updater(PSBT_Role):
    ...
    def add_output_pubkey(self, output_index, pubkey, masterkey_fingerprint, bip32_path):
        ...
        pprint(self.psbt.maps)   
    
    # Blockchain Studio    
    def add_output_network(self, output_index, network):
        self.psbt.maps['outputs'][output_index][PSBT_OUT_NETWORK] = network

    # Blockchain Studio
    # derivs is bytes of the derivation path
    def add_bip32_derivs(self, derivs):
        self.psbt.maps['bip32_derivs'] = derivs
    ...
    def add_partial_signature(self, new_sig, compressed_sec, input_index=None):
        ...
        if input_index is not None:
            ...
            if not self.check_sighash(input_index=input_index, sighash=this_sighash):
                ...
                sighash type {} for this input'.format(this_sighash, 
            ...
            print('Adding partial signature to input {}'.format(input_index))
            self.psbt.maps['inputs'][input_index][PSBT_IN_PARTIAL_SIG+compressed_sec] = new_sig
            print("map added", self.psbt.maps['inputs'][input_index][PSBT_IN_PARTIAL_SIG+compressed_sec])
            pprint(self.psbt.maps['inputs'][input_index])   
...
class Input_Finalizer(PSBT_Role):
    ...
    def __init__(self, serialized_psbt):
        ...
        for i in self.psbt.maps['inputs']:
            ...
            if self._is_witness_input(i):
                ...
                if PSBT_IN_WITNESS_UTXO in i.keys():
                    print('Witness script PSBT_IN_WITNESS_UTXO for input {}'.format(i))
                    new_scriptWitness = self._make_singlesig_script(inp=i, witness=True)
                    print('New scriptWitness: {}'.format(new_scriptWitness))
                    ...
                    i[PSBT_IN_FINAL_SCRIPTSIG] = encode_varint(len(b'\x76\xa9\x14' + new_scriptWitness[1][-20:] + b'\x78\xac')) + b'\x76\xa9\x14' + new_scriptWitness[1][-20:] + b'\x78\xac'                
                if PSBT_IN_WITNESS_SCRIPT in i.keys():
            ...
            # TODO: Test case for this
                ...
                print('Input {} is a P2PKH input'.format(i))
                found_sec = False
                for k in i.keys():
                    print(k)
                    if k[:1] == PSBT_IN_PARTIAL_SIG:
                        print('Found a partial signature')
                        if found_sec:
                            print('Found a second partial signature')
    ...
    def _clear_keyvalues(self, inp):
        ...
        for k in to_delete:
            del inp[k]
        print('Input {} has been cleared'.format(inp))
        return 
                

    def _make_singlesig_script(self, inp, witness=False):
        ...
        if witness:
            print("inp *** ", inp[PSBT_IN_WITNESS_UTXO])
            redeemScript = Script.parse(inp[PSBT_IN_WITNESS_UTXO][-22:])
            print("Script")
            pprint(bytes.hex(redeemScript.serialize()))
            pprint(redeemScript.elements)
        else:
            redeemScript = Script.parse(inp[PSBT_IN_REDEEM_SCRIPT])
        # Make sure this is a singlesig redeemScript by checking for OP_CODE 172, OP_CHECKSIG
        # if redeemScript.elements[-1] != 172:
        #     raise ValueError('Present redeemScript is not singlesig and not understood')
        # total_sigs = int(OP_CODES[redeemScript.elements[-2]][3:])
        total_sigs = 1
        sigs_required = 1
        found_sigs = 0
        print(redeemScript.sec_pubkey())
        ...
        #     raise ValueError("Wrong number of sigs present to satisfy this PSBT input's redeemScript. ", found_sigs, "!= 1")
...
def isValidSignatureEncoding(sig: bytes) -> bool:
    ...
    if (len(sig) < 9): print("Size too small")
    if (len(sig) > 73): print("Size too large")

    # A signature is of type 0x30 (compound).
    if (sig[0] != 0x30): print("Wrong type not 0x30: ", sig[0], bytes(sig[0]))

    # Make sure the length covers the entire signature.
    if (sig[1] != len(sig) - 3): print("Wrong length sig[1] != len(sig) - 3")

    # Extract the length of the R element.
    lenR = sig[3]

    # Make sure the length of the S element is still inside the signature.
    if (5 + lenR >= len(sig)): print("Wrong length 5 + lenR >= len(sig)")
    ...
    # of the elements.
    if ((lenR + lenS + 7) != len(sig)): print("Wrong length (lenR + lenS + 7) != len(sig)")

    # Check whether the R element is an integer.
    if (sig[2] != 0x02): print("Wrong type 0x02")

    # Zero-length integers are not allowed for R.
    if (lenR == 0): print("R length is zero")

    # Negative numbers are not allowed for R.
    if (sig[4] & 0x80): print("R is negative")

    # Null bytes at the start of R are not allowed, unless R would
    # otherwise be interpreted as a negative number.
    if (lenR > 1 and (sig[4] == 0x00) and not(sig[5] & 0x80)): print("R has null bytes")

    # Check whether the S element is an integer.
    if (sig[lenR + 4] != 0x02): print("Wrong type 0x02")

    # Zero-length integers are not allowed for S.
    if (lenS == 0): print("S length is zero")

    # Negative numbers are not allowed for S.
    if (sig[lenR + 6] & 0x80): print("S is negative")

    # Null bytes at the start of S are not allowed, unless S would otherwise be
    # interpreted as a negative number.
    if (lenS > 1 and (sig[lenR + 6] == 0x00) and not(sig[lenR + 7] & 0x80)): print("S has null bytes")
