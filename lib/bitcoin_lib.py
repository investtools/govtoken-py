...
def encode_base58_checksum(s):
    '''Takes a byte sequence s and returns a base58 encoded string with a 4-byte checksum.'''
...
def h160_to_p2wpkh_address(h160, prefix=b'\x05'):
    '''Takes a byte sequence hash160 and returns a p2wpkh address string'''
    # p2wpkh has a prefix of b'\x05' for mainnet, b'\xc4' for testnet
    return encode_base58_checksum(prefix + h160)
...
class PrivateKey:
    ...
    def __init__(self, secret, compressed=False, testnet=True):
        self.secret = secret
        self.point = secret*G
...
class Script:
    ...
    def type(self):
        ...
        if len(self.elements) == 0:
            ...
            print('unknown script type: {}'.format(self.elements))
    ...
    def der_signature(self, index=0):
        ...
        sig_type = self.type()
        if sig_type == 'p2pkh sig'  or 'p2wpkh sig':
    ...
    def address(self, prefix=b'\x00'):
        ...
        if sig_type == 'p2pkh':
            ...
            # hash160 is the 2nd element
            h160 = self.elements[1]
            # convert to p2wpkh address using h160_to_p2wpkh_address
            # (remember testnet)
            return h160_to_p2wpkh_address(h160, prefix)
...
class Tx():
    ...
    def sig_hash_preimage_bip143(self, input_index, hash_type, redeem_script=None):
        ...
        s += int_to_little_endian(tx_in._value, 8)
    ...
    def sig_hash(self, input_index, hash_type):
        ...
        script_pubkey = signing_input.script_pubkey
        sig_type = script_pubkey.type()
        if sig_type == 'p2pkh' or sig_type == 'p2wpkh':
    ...
    def verify_input(self, input_index):
        ...
        for sig_num in range(sigs_required):
            ...
            if tx_in.is_segwit():
                ...
                print('H160', tx_in.script_sig.redeem_script())
                print('H160', h160)
                print('H160', hash160(tx_in.script_pubkey.elements[1]))
                print('H160', tx_in.script_pubkey.elements[1])
                #if h160 != hash160(tx_in.script_pubkey.elements[1]):
                if h160 != tx_in.script_pubkey.elements[1]:
                    print('1')
                    #return False
                #pubkey_h160 = tx_in.script_sig.redeem_script()[-20:]
                pubkey_h160 = tx_in.script_pubkey.elements[1]
                print('POINT', point)
                print('PK 160', pubkey_h160)
                print('PT 160', point.h160())
...
class TxIn():
    ...
    def __init__(self, prev_tx, prev_index, script_pubkey, sequence, witness_program=b'\x00', value=None, script_sig=b''):
        ...
        #else:
        #    self._script_sig = Script.parse(script_sig)
...
class TxOut:
    ...
    def __init__(self, amount, script_pubkey):
        self.amount = amount
        self.script_pubkey = Script.parse(script_pubkey)
        # print("SCRIPT PUBKEY", self.script_pubkey.address())
    ...
    def parse(cls, s):
        ...
        # print("SCRIPT LENGTH", script_pubkey_length)
        script_pubkey = s.read(script_pubkey_length)
        # print("SCRIPT PUBLICK KEY", script_pubkey)
        # return an instance of the class (cls(...))
